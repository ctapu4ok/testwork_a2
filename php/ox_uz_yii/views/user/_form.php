...
<div class="profile">
    <div class="avatar" style="text-align: center">
        ...
        <div class="photo">
            ...
            <?php if (UserHelper::getRememberedAvatar()): ?>
                <img src="<?php echo UserHelper::getRememberedAvatar() ?>" class="remembered">
            <?php elseif ($user->avatar): ?>
                <img src="<?php echo UserHelper::getAvatarBaseUrl() . $user->avatar ?>" draggable="false">
            <?php endif ?>
        </div>
    ...
    <div class="details">
        <div class="form">
            ...
            <?php echo $form->textFieldControlGroup($user, 'username', ['maxlength' => 50, 'disabled' => !$user->isNewRecord]); ?>
            ...
            <?php if ($user->isScenarioType('manager')): ?>
                <?php echo $form->dropDownListControlGroup($user, 'supplier_id', CHtml::listData($user->availableSuppliers, 'id', 'name')); ?>
            <?php endif ?>
            <?php if (($user->isScenarioType('seller') or $user->isScenarioType('cashier')) && $user->isManager(Yii::app()->user->role)): ?>
                <?php if($user->availableShops):?>
                    <?php echo $form->dropDownListControlGroup($user, 'shop_id', CHtml::listData($user->availableShops, 'id', 'name')); ?>
                <?php endif;?>
                <?php echo $form->dropDownListControlGroup($user, 'role', array('seller'=>'Продавец','cashier'=>'Кассир')); ?>
            <?php else : 
                if ($user->isScenarioType('seller')):?>
                    <?php echo $form->dropDownListControlGroup($user, 'shop_id', CHtml::listData($user->availableShops, 'id', 'name')); ?>
                <?php endif ?>
            
            <?php endif ?>
            <?php echo $form->textFieldControlGroup($user, 'ip', ['maxlength' => 20]); ?>
